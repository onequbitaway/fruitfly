...
enum FlyPainter {
    static func draw(
        in ctx: CGContext, at point: CGPoint, heading: Double,
        ...
        ctx.setShadow(
            offset: CGSize(width: 2, height: -3), blur: flying ? 6 : 2,
        ...
        for side in [-1.0, 1.0] {
            ...
            ctx.beginPath()
            ctx.move(to: CGPoint(x: 2, y: 1))
            ctx.addLine(to: CGPoint(x: 14, y: -9))
            ctx.strokePath()
            ctx.beginPath()
            ctx.move(to: CGPoint(x: 7, y: -3))
            ctx.addLine(to: CGPoint(x: 10, y: -11))
            ctx.strokePath()
            ctx.restoreGState()
        }

        shadedEllipse(
            ctx, CGRect(x: -3.6, y: -11, width: 7.2, height: 13),
        ...
        for y in [-7.8, -5.1, -2.5] {
            ...
            ctx.beginPath()
            ctx.move(to: CGPoint(x: -2.8, y: y))
            ctx.addQuadCurve(to: CGPoint(x: 2.8, y: y), control: CGPoint(x: 0, y: y - 1))
            ctx.strokePath()
        }
        shadedEllipse(
            ctx, CGRect(x: -4, y: -1.5, width: 8, height: 9), light: NSColor(white: 0.34, alpha: 1),
            dark: NSColor(white: 0.09, alpha: 1))
        ellipse(ctx, CGRect(x: -3.6, y: 5.5, width: 7.2, height: 5.3), color: NSColor(white: 0.13, alpha: 1))
        for side in [-1.0, 1.0] {
            shadedEllipse(
                ctx, CGRect(x: side * 2.4 - 1.6, y: 6.2, width: 3.2, height: 4.4),
                light: NSColor(red: 0.69, green: 0.24, blue: 0.15, alpha: 1),
                dark: NSColor(red: 0.33, green: 0.09, blue: 0.06, alpha: 1))
            ctx.setStrokeColor(NSColor(white: 0.13, alpha: 1).cgColor)
            ctx.setLineWidth(0.5)
            ctx.beginPath()
            ctx.move(to: CGPoint(x: side, y: 10))
            ctx.addLine(to: CGPoint(x: side * 2, y: 12.4))
            ctx.strokePath()
        }
        if eating {
            ...
            ctx.addLine(to: CGPoint(x: sin(time * 8) * 0.4, y: 13))
            ctx.strokePath()
        }
        ctx.restoreGState()
    }
    ...
    static func crumb(in ctx: CGContext, at point: CGPoint, amount: Double, age: Double) {
        ...
        ctx.scaleBy(x: scale, y: scale)
        ctx.setShadow(
            offset: CGSize(width: 0, height: -1), blur: 2, color: NSColor.black.withAlphaComponent(0.2).cgColor)
        for (x, y, r) in [(-3.0, 1.0, 3.0), (2.0, 3.0, 2.7), (1.0, -2.0, 3.2), (-5.0, -3.0, 1.3), (5.0, 0.0, 1.2)] {
            ...
            shadedEllipse(
                ctx, rect, light: NSColor(red: 1, green: 0.85, blue: 0.48, alpha: 1),
                dark: NSColor(red: 0.67, green: 0.4, blue: 0.14, alpha: 1))
        }
        ctx.restoreGState()
    }
    ...
    static func icon(size: CGFloat = 20) -> NSImage {
        let image = NSImage(size: NSSize(width: size, height: size), flipped: false) { rect in
            ...
            draw(
                in: ctx, at: CGPoint(x: rect.midX, y: rect.midY), heading: .pi / 2,
    ...
    private static func shadedEllipse(_ ctx: CGContext, _ rect: CGRect, light: NSColor, dark: NSColor) {
        ...
        if let gradient = CGGradient(
            colorsSpace: CGColorSpaceCreateDeviceRGB(), colors: [light.cgColor, dark.cgColor] as CFArray,
            locations: [0, 1])
        {
            ctx.drawLinearGradient(
                gradient, start: CGPoint(x: rect.minX, y: rect.maxY), end: CGPoint(x: rect.maxX, y: rect.minY),
                options: [])
        }
        ctx.restoreGState()
    }
